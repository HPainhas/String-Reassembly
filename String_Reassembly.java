...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String removedOverlap = str1.substring(0, str1.length() - overlap);
        String result = removedOverlap + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        Set<String> copy = strSet.newInstance();
        copy.transferFrom(strSet);

        boolean containSubstr = false;

        while (copy.size() > 0) {

            boolean containSubstrAgain = false;
            String analyze = copy.removeAny();

            if (analyze.length() <= str.length()) {

                if (str.contains(analyze)) {
                    containSubstr = true;
                    containSubstrAgain = true;
                ...
                if (analyze.contains(str)) {
                    containSubstr = true;
                }
            }

            if (!containSubstrAgain) {
                strSet.add(analyze);
            }
        }
